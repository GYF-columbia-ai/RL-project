...
from utils.models import *  # 重新覆盖旧的定义
...
class Agent():
    ...
    def load_network(self):
        from utils.models import DQN  # 确保类 DQN 已定义
        import torch.serialization
        torch.serialization.add_safe_globals({'DQN': DQN})  # 明确添加到反序列化白名单

        # 显式设置 weights_only=False
        return torch.load(self.save_path + "_" + self.classe, weights_only=False)
    ...
    def optimize_model(self):
        ...
        print('the input state batch is:', state_batch.shape)
